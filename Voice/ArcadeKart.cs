...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            InputMethodText = GameObject.Find("InputMethod").GetComponent<Text>();
        ...
        {
            InputMethod = "controller";
            InputMethodText.text = InputMethod;

            actions.Add("up", Acc);
            actions.Add("down", Dec);
        ...
        void FixedUpdate()
        {
            ...
            UdpSocket udps = GetComponent<UdpSocket>();
            float bt_input = (float) udps.steeringData.bt_data / 30.0f;
            float pose_input = (float) udps.steeringData.pose_data / 2.0f;
            if (!KeyboardControls)
                print(bt_input + "    " + pose_input);
            ...
            {
                if (KeyboardControls)
                    MoveVehicle(Input.Accelerate, Input.Brake, Input.TurnInput, speedUp, slowDown);
                else
                    MoveVehicle(true, Input.Brake, bt_input + pose_input, speedUp, slowDown);
                speedUp = false;
                slowDown = false;
            }
            GroundAirbourne();
        ...
        void MoveVehicle(bool accelerate, bool brake, float turnInput, bool accl, bool slow)
        {
