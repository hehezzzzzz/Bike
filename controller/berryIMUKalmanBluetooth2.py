...
# to sockets with port number < 1024 but RFCOMM uses channels 1-30 so this will deal with those failures
# and reconnect
for _ in range(20):
    try:
    ...
    print("Failed to connect to target device in allowed time... Exiting")
    time.sleep(5)
    quit()
        

output = 0 #initialize classifier output
...
while True:
    ...
    #amount of steps for the output
    steps = 30
    ...
    expo = 1.1
    #0.7 < expo < 1 makes more initially sensitive, expo > 1 makes less initially sensitive
    #values below 0.7 may have an undesired amount of sensitvity
    
    minout = 3
    #minimum steering output (minimum value is 1)

    #limit kalman filtered angle to the maximum angle
    if (-maxang > kalmanX):
        kalmanX = -maxang
    ...
    #apply exponential to kalman filtered angle
    #also turn the angle into a percentage to be scaled into the desired number of output steps
    if (kalmanX < 0):
        kalmanX = -((-kalmanX/maxang) ** expo)
    else:
        kalmanX = (kalmanX/maxang) ** expo
        
        
    #scale the angle into the desired number of steps
    output = int(kalmanX * steps)
    
    #eliminiate output values below the minimum output value
    if (-minout < output and output < minout):
        output = 0
    ...
    client.sendall(output_str.encode('UTF-8'))
